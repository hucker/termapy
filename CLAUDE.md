...
- Textual is confined to the UI layer: `app.py` (the main app) plus `dialogs/`, `widgets/`, `capture_view.py`, `title_bar.py`, `proto_debug.py`, `info_views.py`, `palette_provider.py`. That list is EXACT, not illustrative — those six modules and two packages are the complete set today. Core modules (`repl.py`, `plugins.py`, `scripting.py`) never import Textual. Any NEW module that imports Textual is UI-layer and must be added to this list. (`pickers.py` is Textual-free — it builds picker *data*, not widgets — so it is not in this list.) **Enforced by `tests/test_architecture.py`**, which pins the set; the complementary runtime check in `test_cli.py` asserts importing `termapy.entry` pulls in no textual module at all, catching TRANSITIVE imports the static pass cannot see.
- **Handlers never touch `ctx.io._write` / `._write_markup`** — use the semantic channels (`result` / `output` / `status`, or their `_markup` variants) so the frontend decides how each kind of message renders. **Enforced by `tests/test_architecture.py`**, matching on the AST attribute rather than the text, so aliasing (`io = ctx.io`) and `getattr` are caught while a comment or docstring naming the primitive is not. (This replaced a CI grep that had both flaws — it once forced a comment to be reworded to appease it.)
...
- Load order: builtins → global → per-config → app hooks (later overrides earlier). The global and per-config layers are resolved by `ReplEngine.resolve_plugins` inside `fire_lifecycle` (`on_app_start` / `on_config_load`) — identically in the TUI, CLI and MCP server, and rebuilt from scratch on every config switch. The order is enforced by the engine, not by call sequence: a folder plugin whose name an app hook owns is skipped with a warning. `TERMAPY_TRUSTED_PLUGINS_ONLY=1` drops both folder layers in every frontend.
- External plugins use `PluginContext` only. `InternalHandle` (`ctx.internal`) is internal/unstable.
- **The reverse direction has a rule too: core may call INTO a plugin, but only its published surface.** Core (`app.py`, `cli.py`, `repl.py`, `mcp/server.py`, `pickers.py`, …) importing a *public* function from `builtins/` is fine — `app.py` calls `find.dismiss()` when live data should close the find bar, and that is the plugin's documented entry point. What is NOT fine is reaching for a private handler or private state (`_handler_clear`, `_VARS`), because a plugin's privates are free to change and the app has no contract with them. **Enforced by `tests/test_architecture.py`** — an `ast` walk over every core module with an explicit allowlist, so a new core→`builtins` import fails `pytest` locally (not just in CI) until it is either moved into core or added to the allowlist as a published name.
- **Data folders come and go through `folders.ensure_folder` / `folders.prune_empty_folders` only.** A data folder (`folders.FOLDERS`) exists while something is in it: every writer calls `ensure_folder` at the write (never at a read — a missing folder lists as empty), and `ReplEngine.fire_lifecycle` prunes empty ones on `on_app_start` / `on_config_load` / `on_app_stop`. A raw `.mkdir(` on a data folder is a bug. A rule, not a migration (Chuck: "if a user empties a folder then it is deleted, that is tidy"). **Enforced by `tests/test_architecture.py`** — an `ast` walk with a short allowlist of non-data-folder sites (cfg root, OS app dirs, launcher files, `mcp/`), probe-proven.
...
- Config dirs: `termapy_cfg/<name>/` with `plugin/`, `ss/`, `run/`, `proto/`, `viz/`, `cap/`, `prof/`, `sym/`, `dev/`, `lib/` data folders, each created on first write and removed when empty at load/stop (`--demo` ships the four it populates). `plugin/`, `dev/` and `lib/` also exist at the cfg root as a global layer; the per-config one wins a name clash.
...
- **Port enumeration takes `source=` / `trust_env=`** (`port_control.resolve_port_source`). Three layers, highest first: explicit `source=`, then `TERMAPY_DEMO_FLEET` unless `trust_env=False`, then real hardware. The env layer stays because the party wanting fake ports is usually not the calling code -- a CI job, a screenshot run, a docs build.
- **Hand a fleet in rather than stubbing `_gather_all_chip_facts`.** A `lambda *a, **kw:` stub swallows every signature change silently (it did -- two of them broke the moment `source=` was added), and an env var cannot express "no ports" or a scripted sequence of snapshots, which is what most of these tests actually want. `source=` is a callable invoked once per gather, so both fall out of it.
- **The `source=` surface reaches the whole port stack, `trust_env=` does not.** `list_ports`, `chip_info`, `chip_field`, `port_info`, `cli_flags.run_info` / `run_ports` / `run_watch`, and `serial_engine._classify_serial_error` all take a keyword-only `source=` and forward it down. They deliberately do *not* take `trust_env=`: deciding whether the environment may speak belongs to the discovery layer, and an injected fleet outranks the variable anyway. No production caller sets `source`; it exists so a caller can drive a handler against a known fleet without owning the machine's ports.
- **Two places keep the older levers, on purpose.** A test that drives `main()` (argparse-to-handler wiring) uses `TERMAPY_DEMO_FLEET`, because injection cannot reach through a process entry point -- that is what layer 2 is for. A test about *enumeration itself* (probing, `_facts_from_port_info`) patches `comports`, because a substitute fleet replaces that layer wholesale and would make the test vacuous. Both are commented where they live (`tests/test_cli.py::TestPortsFlag`, `tests/test_port_noninvasive.py`).
