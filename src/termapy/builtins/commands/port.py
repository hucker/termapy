...
def _apply(ctx: PluginContext, result: port_control.Result) -> None:
    ...
    route through ``ctx.io._write_markup`` so embedded colors render.
    ...
    for text, color in msgs:
        if "[/]" in text:
            ctx.io._write_markup(text)
        elif color:
            ctx.io._write(text, color)
        else:
            ctx.io._write(text)
    if effects:
...
def _handler_chip_list(ctx: PluginContext, args: str) -> CmdResult:
    ...
    if not rows:
        ctx.io._write(f"No chips match '{args.strip()}'.", "yellow")
    ...
    ctx.io._write(header)
    ctx.io._write("-" * len(header))
