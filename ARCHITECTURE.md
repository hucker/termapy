...
Hooks are stored in a flat list in load order (`ReplEngine._lifecycle_hooks`). `fire_lifecycle(name)` filters by name and calls matching handlers in registration order, catching exceptions per-hook so one bad plugin can't prevent later hooks from running. Errors surface through `ctx.status()`. Three core listeners run inside `fire_lifecycle` BEFORE the plugin hooks: the empty data-folder prune (`folders.prune_empty_folders`, on `on_app_start`, `on_config_load` and `on_app_stop`; see "Config and filesystem"), then on `on_app_start` and `on_config_load` external plugin resolution (`ReplEngine.resolve_plugins`: drop every folder-loaded registration, then load `termapy_cfg/plugin/` and `termapy_cfg/<name>/plugin/` again, so a folder plugin's own hook fires in that same pass), then the symbol auto-load (`symbols.session.autoload`). So every frontend resolves plugins and loads `<cfg>.symbols.json` at the same moment -- `on_app_start` fires BEFORE the startup auto-connect in the TUI, CLI and MCP server alike, so an `on_connect` hook or `on_connect_cmd` already has the table -- and a plugin's own hook already sees `ctx.ns("symbols")`. None of the three is a plugin hook (which would drift per frontend) or a `set_context` / `replace_cfg` side effect (which double-loads on the CLI/MCP config switch that rebuilds the ctx).
...
Layers 2 and 3 are resolved by the engine (`ReplEngine.resolve_plugins`) inside `fire_lifecycle` on `on_app_start` and `on_config_load`, identically in the TUI, CLI and MCP server, and rebuilt from scratch on every config switch, so a per-config transform, directive or hook goes away with its config. The order is enforced by the engine rather than by call sequence: a folder plugin whose name an app hook owns is skipped with a warning, however early the frontend registered the hook. `TERMAPY_TRUSTED_PLUGINS_ONLY=1` drops layers 2 and 3 in every frontend; the engine takes the policy as a constructor argument (`trusted_only`) with the frozen env flag as its default, and `global_root` names the folder whose `plugin/` child is layer 2 (None = `config.cfg_dir()`), so a test can drive the whole path against a temp folder.
...
A linker map holds what the firmware defines, never the registers the silicon fixes, so those live in **device files** (`devices.py`; `<cfg>/dev/*.device.json`, plus a global `termapy_cfg/dev/` that loads into every config, per-config overriding by `device` name). The symbol session (`symbols/session.py`) keeps the two inputs apart — `build` (the sidecar) and `devices` — and rebuilds one merged `table` from them through a single installer, so `/sym.import` replaces the build half and cannot drop the board half, and the sidecar never receives a device row. `Symbol` carries the three safety fields the memory commands act on (`rmw`, `access`, `read_effect`); everything a viewer shows (peripheral, description, reset, structured fields) stays on `devices.Register`. Loading a device file also changes what `/mem.*` will *read* at those addresses, because a register is an intrusive probe on live silicon where a variable is not: `devices.registers_in_range` answers the one question the gates ask — does this byte range touch a register — and `devices.distinct_spans` collapses the mode aliases SVD emits (a SERCOM's `CTRLA` is six definitions at one address, 410 such addresses on a PIC32CM5164LE00100, and counting entries rather than spans wrongly refused a single-register read on 47% of that part), so `/mem.dump` refuses a span covering two or more distinct registers, `/mem.str` refuses a scan that could reach any, a `wo` register refuses every reading path (including the read half of `/mem.or` and `/mem.not`), and an allowed peripheral read is logged like a write. Naming one register stays allowed; the *sweep* is what is refused. The gate is address containment rather than per-register knowledge on purpose — `access` is populated in every vendor SVD, but `readAction`, which would say which registers are destructive to read, is empty in all of them, so a safety list is not something termapy could honestly offer. There is deliberately no shipped catalog and no cfg key: a catalog would be a copy-from folder, never a load layer, and a relocatable part cannot load until its file says where it sits (`instances`). The user's own **library** is that copy-from pool made explicit — nothing in it loads. It has the same two layers as `plugin/` and `dev/`: the config's own `<cfg>/lib/` over the shared `termapy_cfg/lib/`, the per-config one winning a name clash (`devices.library_layers`), so a checked-in config folder carries its parts and its references resolve on any machine. Each layer is a `vendor/type/family` tree listed by `/dev.lib` (with its layer) and scanned header-only by `devices.scan_library`, which refuses a file whose `device` field disagrees with its filename. `/dev.import` writes the converted part to the config's own library by default (`to=global` shares it with every config), under `<vendor>/<category>/` with the vendor slugged from the file's own metadata, and drops a reference into `dev/`, so importing a 2 MB MCU leaves 61 bytes in `dev/` rather than a second copy; `/dev.use <part> {at=NAME@ADDR ...}` adds one that is already in the library. A `dev/` file reaches it with `{"ref": "<part>", "instances": [...]}`: the library part keeps the registers and stays pristine while the reference supplies only this board's placement, so one 2517-register MCU file serves every config that uses that chip and re-importing a newer conversion cannot lose a board's bases. A reference may override `instances` and nothing else, since one that could redefine registers would be a fork, and a **relocatable** part must state its own placement rather than inheriting the library part's — those are an example written by whoever prepared the part, and inheriting them would load every register at a plausible but wrong address, which is the same hazard that makes a bare relocatable file an error. `dev/` remains the bill of materials either way — folder presence is still the declaration, which is why this is a file and not the cfg key that was built and deleted during the original device work. The format normalizes whatever a vendor publishes the way `xc32.py` normalizes a linker map: `/dev.import` runs a device converter (`devices/converters/`, CMSIS-SVD built in) and writes the document it returns as `dev/<device>.device.json`, so a converted part and a hand-written one pass the same validation and the file on disk is the source of truth. Symbol and device converters share one record type and lookup (`converters.py`); a plugin file picks its kind with `KIND`, and the engine keys them by `(kind, format)` so the two never collide or cross over.
...
├── dev/                  # device files loaded by every config
├── lib/                  # shared device library: vendor/type/family tree, NOTHING loads
└── <name>/
    ...
    ├── dev/              # device files: the board's registers (*.device.json, or a ref)
    └── lib/              # this config's part library; checked in, the folder travels
```
