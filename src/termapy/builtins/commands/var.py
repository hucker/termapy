...
def _handler_list(ctx: PluginContext, args: str) -> CmdResult:
    ...
    if name:
        ...
        if val is None:
            ...
            if ctx_fn is not None:
                val = ctx_fn()
        if val is not None:
            ctx.io._write_markup(f"  [cyan]$({name})[/] = [green]{val}[/]")
            return CmdResult.ok(value=str(val))
        else:
            ctx.io._write(f"  $({name}) - not defined", "red")
        return CmdResult.ok()
    ...
    for k in sorted(_VARS):
        ctx.io._write_markup(f"  [cyan]$({k})[/] = [green]{_VARS[k]}[/]")
    for k in sorted(_LAUNCH_VARS):
        ctx.io._write_markup(f"  [cyan]$({k})[/] = [green]{_LAUNCH_VARS[k]}[/]  [dim](launch)[/]")
    now = datetime.now()
    for k, fmt in sorted(_DYNAMIC_VARS.items()):
        ctx.io._write_markup(f"  [cyan]$({k})[/] = [green]{now.strftime(fmt)}[/]  [dim](dynamic)[/]")
    for k in sorted(_CONTEXT_VARS):
        ctx.io._write_markup(f"  [cyan]$({k})[/] = [green]{_CONTEXT_VARS[k]()}[/]  [dim](context)[/]")
    return CmdResult.ok()
...
def _handler_set(ctx: PluginContext, args: str) -> CmdResult:
    ...
    value = parts[1]
    _VARS[name] = value
    ctx.io._write_markup(f"  [cyan]$({name})[/] = [green]{value}[/]")
    return CmdResult.ok()
...
def _handler_clear(ctx: PluginContext, args: str) -> CmdResult:
    ...
    count = len(_VARS)
    _VARS.clear()
    ctx.io._write(f"Cleared {count} variable(s).", "green")
    return CmdResult.ok()
