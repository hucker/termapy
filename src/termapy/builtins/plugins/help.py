...
def _render_man_page(ctx: PluginContext, name: str, plugin,
    ...
    if plugin.args:
        ...
        ctx.write_markup(f"  [{_CMD}]{prefix}{name}[/] {args_colored}")

    # DESCRIPTION ─────────────────────────────────────────────────────────────
    # Developer mode shows the handler's Python docstring instead.
    if dev_mode:
        ctx.write_markup("")
        ctx.write_markup(_SECTION_FMT.format(text="DESCRIPTION (developer)"))
    ...
    if required:
        ctx.write_markup("")
        ctx.write_markup(_SECTION_FMT.format(text="REQUIRES"))
    ...
    if see:
        ctx.write_markup("")
        ctx.write_markup(_SECTION_FMT.format(text="SEE ALSO"))
        refs = ", ".join(f"[{_CMD}]{prefix}{n}[/]" for n in see)
        ctx.write_markup(f"  {refs}")
...
def _handler(ctx: PluginContext, args: str) -> CmdResult:
    ...
    for source in sorted_sources:
        ...
        for cmd_name, plugin in sorted(groups[source], key=lambda x: x[0]):
            ctx.write_markup(_landscape_row(prefix, cmd_name, plugin, cmd_w))
    ...
    if directives:
        ...
        for d in directives:
            ...
            ctx.write_markup(f"{cmd_col}  {d.help}")
...
def _render_target(ctx: PluginContext, cmd_w: int | None = None) -> None:
    ...
    for cmd_name in sorted(names):
        ...
        ctx.write_markup(f"{cmd_col}{args_col}  {tc.help}")
...
def _render_scripts(ctx: PluginContext, scripts: list, prefix: str,
    ...
    for path in scripts:
        ...
        ctx.write_markup(f"{cmd_col}  {desc}")
