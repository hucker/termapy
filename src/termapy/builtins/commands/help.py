...
def _write_docstring(ctx: PluginContext, docstring: str) -> None:
    ...
    for i, line in enumerate(lines):
        ...
        if i == 0 and has_summary:
            ctx.io._write_markup(_TITLE_FMT.format(text=text))
        elif (m := _LABEL_RE.match(text)):
            ctx.io._write_markup(_LABEL_FMT.format(
                indent=m.group(1),
                label=m.group(2) + m.group(3),
                rest=m.group(4).rstrip(),
            ))
        else:
            ctx.io._write(f"  {text}")
...
def _render_man_page(ctx: PluginContext, name: str, plugin,
    ...
    # NAME ────────────────────────────────────────────────────────────────────
    ctx.io._write_markup(_SECTION_FMT.format(text="NAME"))
    help_line = interpolate_help(plugin.help, prefix)
    ctx.io._write_markup(f"  [{_CMD}]{prefix}{name}[/] - {help_line}")

    # SYNOPSIS ────────────────────────────────────────────────────────────────
    if plugin.args:
        ctx.io._write_markup("")
        ctx.io._write_markup(_SECTION_FMT.format(text="SYNOPSIS"))
        args_colored = _color_args(plugin.args)
        ctx.io._write_markup(f"  [{_CMD}]{prefix}{name}[/] {args_colored}")

    # DESCRIPTION ─────────────────────────────────────────────────────────────
    # Developer mode shows the handler's Python docstring instead.
    if dev_mode:
        ctx.io._write_markup("")
        ctx.io._write_markup(_SECTION_FMT.format(text="DESCRIPTION (developer)"))
        docstring = getattr(plugin.handler, "__doc__", None)
        if docstring:
            _write_docstring(ctx, docstring)
        else:
            ctx.io._write("  (no docstring)")
    else:
        ...
        if lh:
            ctx.io._write_markup("")
            ctx.io._write_markup(_SECTION_FMT.format(text="DESCRIPTION"))
            ...
            for line in lh.strip().splitlines():
                ctx.io._write_markup(f"  {line}")

    # FLAGS ───────────────────────────────────────────────────────────────────
    rows = _canonical_flags(plugin)
    if rows:
        ctx.io._write_markup("")
        ctx.io._write_markup(_SECTION_FMT.format(text="FLAGS"))
        for canonical, aliases, desc in rows:
            names = ", ".join([canonical, *aliases])
            ctx.io._write_markup(f"  [{_OPT}]{names}[/] - {desc}")

    # REQUIRES ────────────────────────────────────────────────────────────────
    required = _required_capability_rows(plugin.needs)
    if required:
        ctx.io._write_markup("")
        ctx.io._write_markup(_SECTION_FMT.format(text="REQUIRES"))
        for cap_name, hint in required:
            ctx.io._write_markup(f"  [{_OPT}]{cap_name}[/] - [{_SEP}]{hint}[/]")
    ...
    ctx.io._write_markup("")
    ctx.io._write_markup(_SECTION_FMT.format(text="AVAILABLE"))
    cells: list[str] = []
    ...
    ctx.io._write_markup("  " + "   ".join(cells))
    if missing_by_env:
        ...
        ctx.io._write_markup(f"  [{_SEP}]({'; '.join(notes)})[/]")
    ...
    if plugin.children:
        ...
        if children:
            ctx.io._write_markup("")
            ctx.io._write_markup(_SECTION_FMT.format(text="SUBCOMMANDS"))
            cmd_w = _compute_cmd_w([n for n, _ in children], prefix)
            for child_name, child in children:
                ctx.io._write_markup(_landscape_row(prefix, child_name, child, cmd_w))

    # SEE ALSO ────────────────────────────────────────────────────────────────
    see = _siblings_for_see_also(name, plugins)
    if see:
        ctx.io._write_markup("")
        ctx.io._write_markup(_SECTION_FMT.format(text="SEE ALSO"))
        refs = ", ".join(f"[{_CMD}]{prefix}{n}[/]" for n in see)
        ctx.io._write_markup(f"  {refs}")

    # Source annotation for non-built-in plugins (preserved from old renderer).
    if plugin.source not in ("built-in", "app"):
        ctx.io._write_markup("")
        ctx.io._write_markup(f"  [{_SRC}](source: {plugin.source})[/]")
...
def _render_target_man_page(ctx: PluginContext, tc) -> None:
    ...
    # NAME ────────────────────────────────────────────────────────────────────
    ctx.io._write_markup(_SECTION_FMT.format(text="NAME"))
    ctx.io._write_markup(f"  [{_CMD}]{tc.name}[/] - {tc.help}")

    # SYNOPSIS ────────────────────────────────────────────────────────────────
    if tc.args:
        ctx.io._write_markup("")
        ctx.io._write_markup(_SECTION_FMT.format(text="SYNOPSIS"))
        ctx.io._write_markup(f"  [{_CMD}]{tc.name}[/] {_color_args(tc.args)}")
    ...
    ctx.io._write_markup("")
    ctx.io._write_markup(f"  [{_SRC}](source: target device)[/]")
...
def append_files_section(
    ...
    ctx.io._write_markup("")
    ctx.io._write_markup(_SECTION_FMT.format(text=title))
    if not files:
        ctx.io._write_markup("  (none)")
        return
...
def _handler(ctx: PluginContext, args: str) -> CmdResult:
    ...
    for source in sorted_sources:
        ...
        if not first:
            ctx.io._write_markup("")
        first = False
        ctx.io._write_markup(f"[{_SEP}]-- {label} --[/]")
        for cmd_name, plugin in sorted(groups[source], key=lambda x: x[0]):
            ctx.io._write_markup(_landscape_row(prefix, cmd_name, plugin, cmd_w))

    if directives:
        ctx.io._write_markup("")
        ctx.io._write_markup(f"[{_SEP}]-- Directives --[/]")
        for d in directives:
            cmd_col = _pad(f"  [{_CMD}]{d.name}[/]", cmd_w + 2)
            ctx.io._write_markup(f"{cmd_col}  {d.help}")

    if script_only:
        ctx.io._write_markup("")
        ctx.io._write_markup(f"[{_SEP}]-- Script Commands (.run files only) --[/]")
...
def _render_target(ctx: PluginContext, cmd_w: int | None = None) -> None:
    ...
    ctx.io._write_markup(f"[{_SEP}]-- Target Device --[/]")
    for cmd_name in sorted(names):
        ...
        ctx.io._write_markup(f"{cmd_col}{args_col}  {tc.help}")
