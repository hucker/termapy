...
class InternalHandle:
    ...
    # library_layers() -> [(folder, label)]: the device libraries, global
    # then per-config (devices.library_layers), for the same reason as
    # reload_devices -- only the engine knows the global root, and a
    # handler resolving it itself would read the real termapy_cfg/ from
    # under a test that deliberately overrode it.
    library_layers: Callable | None = None
